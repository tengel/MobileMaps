	...
	else
	{
		...
		if (records.size() > 0)
		{
			...
			for (unsigned int i = 0; i < records.size(); ++i)
			{
				...
				writeUShort(dat, records[i].x);
				writeUShort(dat, records[i].y);
			}
		}
...
void Index::add(const string& title, unsigned short x, unsigned short y)
{
	Record R;
	R.title = title;
	R.x = x;
...
void Index::write(const string& file) const
{
...
void writeUShort(string& s, unsigned short i)
{
	unsigned short be = (i >> 8 & 0x00FF) | ( i << 8 & 0xFF00);
	s += string(reinterpret_cast<char*>(&be), 2);
}
