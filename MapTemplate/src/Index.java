...
public class Index extends Canvas
{
	...
	private class Loader implements Runnable
	{
		...
		public void run()
		{
			...
			while(!quit)
			{
				...
				{
					int numSent = 0;
					InputStream is = getClass().getResourceAsStream("/index.dat");
					if (is == null) // No index.
						return;					
					...
					try
					{
						while (numSent < 10) // TODO: This code smells.
						{
							...
							else
							{
								...
								for (int i = 0; i < numRecords; ++i)
								{
									...
									ent.x = din.readUnsignedShort();
									ent.y = din.readUnsignedShort();
	...
	protected void paint(Graphics g)
	{
		...
		for (int i = 0; i < entries.size() && (i + 2) * font.getHeight() < height; ++i)
		{
			...
			g.drawString(((IndexEntry)entries.elementAt(i)).name, 3, i * font.getHeight(),
